...
if conn:
    ...
    with conn.cursor() as cursor:
        cursor.execute(
            "DROP TABLE IF EXISTS election_constituency;"
        )
        conn.commit()
    ...
    print(
        f"{len(df)} cleaned records inserted successfully."
    )
...
# Rename columns to match what ElectionCharts /
...
# turnout_2016
#
# This mapping bridges the two without changing
# election_charts.py or retraining the models.
# -------------------------------------------------------
...
missing = [
    ...
    if c not in df_for_charts.columns
]

if missing:
    st.error(
        f"Data is missing required columns for charts: {missing}"
    )
    st.stop()
...
# ---------------------------------------------------------
# RIGHT: Turnout Scenario Simulator
# ---------------------------------------------------------
